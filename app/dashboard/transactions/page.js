...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border",
        cls,
      )}
    >
...
export default function LedgerPage() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (statusFilters.length > 0)
      data = data.filter((t) => statusFilters.includes(t.status));
  ...
  const metrics = useMemo(() => {
    ...
    const flagged = filteredTransactions.filter(
      (t) => t.status === "FLAGGED",
    ).length;
    const cleared = filteredTransactions.filter(
      (t) => t.status === "CLEARED",
    ).length;
    const avg =
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1.5 font-medium">
            {ALL_TRANSACTIONS.length.toLocaleString()} records · virtualized for
            60fps · URL-synced filters
          </p>
        </div>
        ...
        {[
          {
            ...
            subtext: `${metrics.count.toLocaleString()} records`,
            icon: DollarSign,
            trend:
              metrics.count < ALL_TRANSACTIONS.length ? undefined : "+12.4%",
          },
          {
            label: "Flagged",
          ...
          {
            label: "Cleared",
            value: metrics.cleared.toLocaleString(),
            subtext: "Verified clean",
            icon: BadgeCheck,
          },
          {
            label: "Avg Ticket",
            value: fmt(metrics.avg),
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-2.5 bg-secondary/5 border-b border-border flex items-center justify-between">
          <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">
            ...
            {ALL_TRANSACTIONS.length.toLocaleString()} records
          </span>
          {hasActiveFilters && (
            <span className="text-[10px] font-bold text-primary">
              Filters active · URL synced
            </span>
          )}
        </div>
        ...
        <div className="flex items-center px-6 py-3 bg-secondary/30 border-b border-border sticky top-0 z-10">
          <div className="w-10 flex-shrink-0">
            ...
            >
              {selectedIds.size > 0 && (
                <div className={cn(
                  "w-2 h-0.5 bg-primary-foreground rounded-full",
                  selectedIds.size === filteredTransactions.length && "w-2 h-2 rounded-sm"
                )} />
              )}
            </button>
          </div>
          {COLUMNS.map((col) => (
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 text-[10px] font-black text-muted-foreground uppercase tracking-widest hover:text-foreground transition-colors",
                col.cls,
              )}
            >
              {col.label}
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedIds.has(t.id) && "bg-primary/10"
                    )}
                  ...
                  >
                    <div className="w-10 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                      <button 
                        ...
                        className={cn(
                          "w-4 h-4 rounded border flex items-center justify-center transition-colors",
                          selectedIds.has(t.id) ? "bg-primary border-primary" : "border-border hover:border-primary/50"
                        )}
                      >
                        {selectedIds.has(t.id) && <div className="w-2 h-2 bg-primary-foreground rounded-sm" />}
                      </button>
                    </div>
