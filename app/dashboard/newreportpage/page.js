...
export default function NewReportOnePage() {
  ...
  // --- REUSABLE SIDEBAR COMPONENT (WHITE-LABEL READY) ---
  ...
  return (
    <motion.div
      ...
      className="max-w-6xl mx-auto space-y-24 pb-32 pt-8 px-4 lg:px-0"
    >
      
      {/* --- STEP 1: CHOOSE TEMPLATE --- */}
      <section id="step1" className="grid grid-cols-1 lg:grid-cols-3 gap-8 scroll-mt-24">
        <div className="lg:col-span-2 space-y-6">
          <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1, duration: 0.36 }} className="flex items-center gap-3">
          ...
          >
            {templates.map((t) => (
              <motion.div
                key={t.id}
                variants={{ initial: { opacity: 0, y: 14 }, animate: { opacity: 1, y: 0, transition: { duration: 0.3 } } }}
