...
const generateAmount = () => {
  ...
  if (bucket < 0.75)
    return faker.number.float({ min: 2000, max: 15000, fractionDigits: 2 });
  if (bucket < 0.92)
    return faker.number.float({ min: 15000, max: 80000, fractionDigits: 2 });
...
export const generateTransactions = (count = 10000) => {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const approvedBy =
      finalStatus === "CLEARED" ? faker.person.fullName() : null;
...
export const generateARRData = (months = 24) => {
  ...
  return Array.from({ length: months }, (_, i) => {
    ...
    const dip =
      Math.random() < 0.1 ? faker.number.float({ min: -0.02, max: 0 }) : 0;
...
export const generateCashFlowData = (weeks = 52) => {
  ...
  return Array.from({ length: weeks }, (_, i) => {
    ...
    const inflow = faker.number.float({
      min: 800000,
      max: 3500000,
      fractionDigits: 0,
    });
    const outflow = faker.number.float({
      min: 600000,
      max: 2800000,
      fractionDigits: 0,
    });
...
export const generateDailyVolume = (days = 365) => {
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    const volume = faker.number.float({
      min: count * 1000,
      max: count * 15000,
      fractionDigits: 0,
    });
...
export const generateOverviewStats = () => {
  ...
  return {
    totalAssets: formatCurrency(
      faker.number.float({ min: 1.1e9, max: 1.5e9, fractionDigits: 0 }),
    ),
    liquidity: `${faker.number.float({ min: 3.8, max: 5.2, fractionDigits: 1 })}x`,
    operatingMargin: `${faker.number.int({ min: 68, max: 78 })}%`,
    arr: formatCurrency(
      faker.number.float({ min: 38e6, max: 50e6, fractionDigits: 0 }),
    ),
    burnRate: formatCurrency(
      faker.number.float({ min: 1.2e6, max: 2.5e6, fractionDigits: 0 }),
    ),
    flaggedCount: faker.number.int({ min: 38, max: 65 }),
    pendingApprovals: faker.number.int({ min: 12, max: 30 }),
    avgTicketSize: formatCurrency(
      faker.number.float({ min: 2000, max: 4500, fractionDigits: 0 }),
    ),
 ...
 * }}
 */
export const generateAllMockData = (transactionCount = 5000) => {
  const transactions = generateTransactions(transactionCount);
  const overviewStats = generateOverviewStats();
  
  return {
    recentTransactions: transactions.slice(0, 50),
    revenueData: generateARRData(24),
    chartData: generateDailyVolume(60), // Last 60 days for smooth area chart
    stats: {
      margin: overviewStats.operatingMargin.replace('%', ''),
      growth: overviewStats.yoyGrowth,
      assets: overviewStats.totalAssets,
      liquidity: overviewStats.liquidity,
    },
    raw: {
      transactions,
      overviewStats
